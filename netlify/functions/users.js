...
export default async (req, context) => {
  ...
  try {
    ...
    if (method === "POST" && action === "register") {
      ...
      const user = { alias: key, password, avatarBase: avatarBase || 0, unlockedItems: [], highscores: {}, logs: [], bingo: [], completedDaily: {}, joinedAt: new Date().toISOString() };
    ...
    if (method === "POST" && action === "addlog") {
      ...
      await db.execute({
        ...
        args: [key, log.date, JSON.stringify(log.exercises || []), log.points || 0, log.minutes || 0,
          log.bingo ? 1 : 0, log.bingoFootball ? 1 : 0, log.dailyChallenge ? 1 : 0],
      });
      return new Response(JSON.stringify({ ok: true }), { status: 200, headers });
    }
    ...
    if (method === "PUT" && action === "editlog") {
      ...
      await db.execute({
        ...
        args: [log.date, JSON.stringify(log.exercises || []), log.points || 0, log.minutes || 0, logId],
      });
      return new Response(JSON.stringify({ ok: true }), { status: 200, headers });
    }
...
async function getCompletedDaily(db, alias) {
  ...
  const obj = {};
  result.rows.forEach(r => { obj[r.date] = r.challenge_id; });
  return obj;
}
