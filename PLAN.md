...
- [x] `jsx-a11y` lint sweep: ran the **strict** ruleset (recommended already passed) — code was clean, so the real work was the gaps the linter can't catch. Added accessible names to unlabeled inputs (Log exercise/activity, log-history edit, challenge amount, invite name, profile name), `aria-label`/`aria-pressed` on ActivityFeed pagination arrows + reaction toggles, dialog accessible names (`aria-label`) on LevelUpModal/BuddyCelebration/Bonus/Adult bingo modals/AlbumLightbox, `role="dialog"` on the CardDetailModal panel, and `aria-label` on the bingo cancel `✕`.
- [x] Keyboard navigation pass on the main flows: confirmed all interactive elements are real `<button>`/`<a>` (bingo board cells are display-only; marking happens via real buttons in `ChallengeList`); all modals close on Escape + backdrop. Restored the focus ring on three inputs that had `outline-none` with no `focus:` replacement (log-history, challenge amount, profile name).
- [x] Quick cross-device check: Next.js 15 injects the default `width=device-width` viewport (app is phone-first by design); removed a stale "TopBar lands in S5" comment from `layout.tsx`.
- [x] Fix any bugs surfaced during the port: none new beyond the a11y gaps above (the port sessions already carried `fix:`-style corrections, e.g. log-history preserving summer-activity values).
...
- [x] Playwright E2E suite (6 specs, all green against the real stack + temp Turso):
  - `register-log` — register via invite (UI) → log an exercise (UI) → points shown on profile
  - `buddy` — create → accept → both log → challenge completes (full API stack)
  - `photos` — upload (UI, compress→POST) → fetch bytes back through the auth-gated endpoint
  - `admin-invite` — admin creates an invite (UI) → new user registers with the code (UI)
  - `auth` — unauthed redirects (2 cases, from S4)
  - Setup goes through the real Route Handlers via a cookie-sharing request context; `e2e/helpers.ts` holds the primitives. Config loads `.env.local` and runs single-worker (shared DB + in-memory rate limiter).
- [x] CI: `test:coverage` + coverage artifact on the verify job; added a **secrets-gated** `e2e` job (installs Chromium, runs Playwright; auto-skips when DB/admin secrets aren't configured, e.g. fork PRs).
- [x] `README.md` rewrite: new Next 15 + TS + Tailwind stack, env vars, scripts, architecture, security model, deployment.
- [x] `CONTRIBUTING.md`: Conventional Commits, branch strategy, local checks, hooks, test policy.
- [ ] **Pre-launch DB (Emil)**: rotate the Turso token; run "Nollställ säsong" to clear test data (incl. the `e2e_*` users this suite created + `s7smoke`); set season start + countdown date.
- [ ] **Merge `rewrite/next` → `master`, deploy, smoke-test production (Emil will do the merge).**
...
- **2026-05-31 (Session 12 — code/docs done, launch ops left to Emil)** — Built the Playwright E2E suite (`e2e/`): 6 specs covering register→log→stats, buddy create→accept→complete, photo upload→byte round-trip, admin invite→register, and the S4 auth redirects — **all green** against the running app + temp Turso. Helpers (`e2e/helpers.ts`) drive auth/setup through the real Route Handlers via a cookie-sharing request context; `playwright.config.ts` loads `.env.local` and runs single-worker. CI: switched verify to `test:coverage` + coverage artifact, added a secrets-gated `e2e` job (skips cleanly without DB/admin secrets). Rewrote `README.md` for the Next 15 stack and added `CONTRIBUTING.md`. **Bugs fixed during QA:** (1) **launch-critical** — legacy `album_photos.image_data NOT NULL` column broke every photo upload (incl. prod); `initDb` now detects the column (PRAGMA) and the route supplies `''` for it (non-destructive — Emil chose this over a destructive `DROP COLUMN`); regression test added. (2) Photo storage had no local backend, so dev/E2E uploads failed — added a `NODE_ENV`-gated filesystem fallback (`.photos-dev/`, gitignored); prod still uses Netlify Blobs. (3) Deleted dead code: old `src/screens/*.jsx`, `src/index.css`, `src/components/PlaceholderScreen.tsx`. typecheck + lint + 136 unit tests + build + 6 E2E all green. **Remaining for Emil:** rotate the Turso token, run "Nollställ säsong" (clears the `e2e_*`/`s7smoke` test data), set season-start + countdown, then **merge `rewrite/next` → `master`** and smoke-test prod.
...
- **2026-05-30 (Session 11 ✅ — a11y & QA pass)** — Time-boxed correctness pass on `rewrite/next`. ESLint `jsx-a11y` **recommended** already passed, so ran the **strict** ruleset too (also clean) and focused on what the linter can't see, verifying each finding by hand (an Explore sweep had several false positives — e.g. the `×` close buttons in the bonus/adult/intro modals already had `aria-label`). Fixes (16 files, all `fix:`-grade): accessible names (`aria-label`) on previously unlabeled inputs (`ActivityInput`, `ExerciseInput` ×3, `ChallengeForm` amount, `InviteManager` name, `ProfileHeader` name, log-history edit); `aria-label` + `aria-pressed` on `ActivityFeed` pagination arrows and reaction toggles; dialog accessible names on `LevelUpModal`/`BuddyCelebration`/`BonusBingoModal`/`AdultBingoModal`/`AdultIntroModal`/`AlbumLightbox`; `role="dialog"` + name on the `CardDetailModal` panel; `aria-label` on the bingo cancel `✕`. Restored the visible focus ring on 3 inputs that had `outline-none` with no `focus:` replacement (log-history, challenge amount, profile name). Keyboard nav verified (all interactives are real buttons/links — bingo board cells are display-only, marking is via real buttons in `ChallengeList`; modals Escape/backdrop-close). Mobile: Next 15 default `width=device-width` viewport (phone-first); removed a stale comment in `layout.tsx`. No new bugs surfaced. 135 tests unchanged; typecheck + lint (incl. strict a11y) + test + build all green. **Next: Session 12 (E2E tests, CI, launch).**
