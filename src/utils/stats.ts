...
export function computeStats(user: User): Stats {
  ...
  const exerciseHighscores: Record<string, number> = { ...user.highscores };
  let totalTouch = 0;
  logs.forEach((l) => {
  ...
  // streak — only count logs meeting the minimum threshold (5 min OR 30 touch)
  ...
  if (qualifyingDays.length > 0) {
    ...
    const diff = (todayD.getTime() - last.getTime()) / 86400000;
    streak = diff <= 1 ? cur : 0;
  }
  ...
  for (let row = 0; row < 5; row++) {
    if (
      Array.from({ length: 10 }, (_, c) => BINGO[row * 10 + c]).every(
        (b) => b && bingoDoneSet.has(b.id),
      )
    )
      bingoLines++;
  }
  for (let col = 0; col < 10; col++) {
    if (
      Array.from({ length: 5 }, (_, r) => BINGO[r * 10 + col]).every(
        (b) => b && bingoDoneSet.has(b.id),
      )
    )
      bingoLines++;
  ...
  // Summer streaks — days in a row with at least 1 of an activity
  function calcSummerStreak(field: 'iceCream' | 'swim' | 'pages'): number {
    ...
    for (let i = 0; i < days.length; i++) {
      c =
        i === 0
          ? 1
          : (new Date(days[i]!).getTime() - new Date(days[i - 1]!).getTime()) / 86400000 === 1
            ? c + 1
            : 1;
    }
    if (days.length === 0) return 0;
    return (new Date(today).getTime() - new Date(days[days.length - 1]!).getTime()) / 86400000 <= 1
      ? c
      : 0;
  }
