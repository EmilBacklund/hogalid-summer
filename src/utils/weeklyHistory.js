...
export function getWeeklyChallengeForDate(date, seasonStart) {
  const base = seasonStart || date;
  const weekNum = Math.floor((new Date(date).getTime() - new Date(base).getTime()) / (86400000 * 7));
...
function computeWeekValue(allUsers, weekStart, weekEnd, challengeType) {
...
export function computeWeeklyHistory(allUsers, seasonStart) {
...
// Saves previous week's result to DB (called as background task)
export async function saveWeeklyResult(allUsers, seasonStart) {
  if (!seasonStart || !allUsers || allUsers.length === 0) return;
