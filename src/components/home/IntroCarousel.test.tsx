...
describe('IntroCarousel swipe navigation', () => {
  ...
  it('shows the home-screen tip page', () => {
    render(<IntroCarousel onClose={vi.fn()} />);
    const dialog = screen.getByRole('dialog');
    for (let i = 0; i < 8; i++) swipe(dialog, 200, 120); // walk to the tip page (9/10)
    expect(screen.getByText('9/10')).toBeInTheDocument();
    expect(screen.getByText('Lägg appen på hemskärmen')).toBeInTheDocument();
  });
  ...
  it('closes when swiping left past the last page', () => {
    ...
    const dialog = screen.getByRole('dialog');
    for (let i = 0; i < 9; i++) swipe(dialog, 200, 120); // walk to last page (10/10)
